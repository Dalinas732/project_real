...
class Temperature_Data:
    ...
    def check_length_of_month(self,count,month,year): # This function will help spot missing or incomplete data
        ...
        if month in {1,3,5,7,8,10,12}:
            correct = 31
            if (month,year) not in {(8,"2000")}:
                assert count == correct, f"Number of days in month = {month} different than expected (31). Year = {year}."
            else:
                assert count == correct, f"Special Month: August of 2000 is incomplete."
        elif month in {4,6,9,11}:
            correct = 30
            if (month,year) not in {(6,"2000")}:
                assert count == correct, f"Number of days in month = {month} different than expected (30).Year = {year}."
            else:
                assert count == correct, f"Special Month: June of 2000 is incomplete."
        elif month == 2:
            if int(year)%4 == 0:
                correct = 29
            else:
                correct = 28
            if year not in {"2000"}:
                assert count in {28,29}, f"Number of days in February different than expected. Year = {year}."
        else:
    ...
    def collect_month(self,month,year,mode=None): ###!!! Will not handle NaNs correctly atm !!!####
        ...
        df = self.dfs[year]
        temps_array = np.array(df.iloc[:,index])
        temps_list = []
        day = 1
        for temps in temps_array:
            if np.isnan(temps) == False:  # Checks to make sure Data is clean
                ...
                print(f"Warning: This month={month}, {year} contains NaNs.") ### Does not raise error
            elif np.isnan(temps) == True and day > 28:
                pass
            else:
                raise ValueError("Value was not a float or a NaN.")
            day+=1
