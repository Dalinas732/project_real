...
class Temperature_Data:
    ...
    def _check_zeros(self,data): # This checks the month for a low of zero.
        for i in range (data.size-1):
            if data[i] == 0:
                return True
        return False
    
    def _fix_zeros(self,frame,year): # This fixes the zeros such that it is correctly correlated with the high and averages
        months = np.arange(12) + 1
        for month in months: ### Goes through all months in a given year
            self.first = False # This is important so the function doesn't call back to _fix_zeros for an infinite loop
    ...
    def collect_month(self,month,year,mode=None): ## Collects the temperaturees of all the days in a given month in a given year
        ...
        if self.first == True: # If not being called from _check_zeros
