...
class Temperature_Data:
    ...
    def collect_day_average(self,month,day): # This function collects all daily averages for a given day between 1960-2024
        temps = []
        day-=1                # Bc rows start at 0 subtract 1
        month = 3*month - 2   # Only looks at average column for a given month
        for year in self.years:
            df = self.dfs[year]
            avg = np.array(df.iloc[day,month])
            if np.isnan(avg) == False:  # Checks to make sure Data is clean
                temps.append(avg)
            elif np.isnan(avg) == True:
                raise ValueError("Data was read as NaN")
            else:
                raise ValueError("Data was flagged as both a Nan and not a Nan.")
        temps = np.array(temps)
    ...
    def collect_day_low(self,month,day): # This function collects all daily lows for a given day between 1960-2024
        temps = []
        day-=1
